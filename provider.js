/**
 * 本文件说明:验证登陆爬取课程表数据返回parser
 * 1.适配于_安徽职业技术学院_本部_音高八度
 * 2. 联系:QQ:3591523326
 * 3.适配时间:2024/11/23
 * 4.参考文档：xiaoxiao_schedule
 * 5.主要修改了课程表获取方式以及解析方式
 */
...
function AIScheduleLoading({
  ...
  async function getSemestersId(preUrl, courseTableCon) {
    ...
    let data = 'dataType=semester&limit=all'; // 添加 limit 参数
    let semestersText = await request("post", data, xqurl, '加载学期中');
    console.log(semestersText); // 打印服务器返回的数据
  
    let semesters = semestersText.split('\n');
    let semesterIndexTag = 0;
  
    for (let line of semesters) {
      if (line.trim()) { // 确保行不为空
    ...
    // 只保留最后四个学期
    selectList = selectList.slice(-4);
    semesterIds = semesterIds.slice(-4);
  ...
  async function scheduleHtmlProvider(
    ...
    if (message.length != 0) {
      ...
      let errText = `
        错误信息，请反馈给开发者
        此页面截图，加群:628325112或QQ:3591523326，反馈错误信息
        错误：${message},
        url:${preUrl}
        `
      AIScheduleAlert({
